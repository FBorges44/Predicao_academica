...
const API = window.APP_CONFIG?.API_BASE_URL || 'http://127.0.0.1:8000/api';
...
document.addEventListener('DOMContentLoaded', () => {
  el('addForm').addEventListener('submit', async e => {
    ...
    try {
      ...
      showStatus('✅ Aluno salvo com sucesso!', 'success');
...
function mostrarPredicao(data, dados) {
  ...
  const corMap = { high: 'var(--risk-high)', medium: 'var(--risk-med)', low: 'var(--risk-low)' };
  ...
  // Fatores que influenciaram — explicação legível
  ...
  el('predFactors').innerHTML = fatores.map(f =>
    `<div style="font-size:var(--fs-xs);color:var(--text-muted);margin-top:3px">
       <span style="color:${f.cor}">${f.icone}</span> ${f.texto}
     </div>`
  ).join('');
...
function gerarFatores(d, prob) {
  ...
  return fatores.slice(0, 6); // Máximo 6 fatores exibidos
}
